...
func TestComposerUsesReadableImagePreviewAndCompactFileChip(t *testing.T) {
	handler := Handler()
	page := httptest.NewRecorder()
	handler.ServeHTTP(page, httptest.NewRequest(http.MethodGet, "/", nil))
	...
	app := httptest.NewRecorder()
	handler.ServeHTTP(app, httptest.NewRequest(http.MethodGet, "/app.js", nil))
	for _, contract := range []string{`URL.revokeObjectURL(previewURL)`, `URL.createObjectURL(selected)`, `const isImage = Boolean(previewURL) && !previewFailed`, `composer.classList.toggle("has-attachment", Boolean(selected))`, `fileChip.classList.toggle("is-image", isImage)`, `filePreview.addEventListener("error"`, `filePreview.currentSrc !== previewURL`} {
	...
	stylesheet := httptest.NewRecorder()
	handler.ServeHTTP(stylesheet, httptest.NewRequest(http.MethodGet, "/app.css", nil))
	for _, contract := range []string{`.composer.has-attachment { border-radius: 32px; }`, `.file-chip { position: relative; width: fit-content;`, `.file-preview { display: none; width: auto; height: auto; max-width: 220px; max-height: 96px; object-fit: contain;`, `.file-chip.is-image { width: fit-content; height: auto; min-height: 40px; padding: 3px;`, `.file-chip.is-image:hover button`} {

...
本文件是英文 `AGENTS.md` 的中文译文。项目规则以英文 `AGENTS.md` 为唯一权威，`CLAUDE.md` 必须始终是指向它的软链接；两份不一致时以英文为准，并且必须在同一个 commit 里一起改。
...
- **Decided（Max，2026-09-15）**，取代 2026-09-11 的「不上架 App Store」：iOS 将在除中国大陆以外的所有 App Store 地区上架。中国大陆区需要 ICP 备案号；Ferry 没有备案，选择不上大陆区，而不是去办理备案。目前还没有提交任何审核。每次提交仍需用户明确授权；App 审核需要能让审核员连上 Ferry Server 使用 App，而 Ferry 不支持暴露到公网。**Decided（Max，2026-09-15）**：第一次提交给审核员提供演示视频，并在审核备注里说明如何运行 Ferry Server；不提供公网演示服务器，也不在 App 里加演示模式。如果因此被拒，下一个待决定的方案是只供审核、限时开放的演示服务器。分发所需的 Team ID、ExportOptions 与流水线都在账号侧的私有知识库，不进本仓库。
...
- **Decided（Max，2026-09-13）**：所有文档都提供英文和简体中文两份。英文放在原路径，中文放在同名的 `*.zh-Hans.md`，两份在标题下互相链接。改其中一种语言时，另一种在同一个 commit 里同步修改。

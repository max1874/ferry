...
- **完成标准**：Web 时间线和 Devices 面板按 `iphone / ipad / mac / android / windows / browser` 显示对应设备图标；已有 macmini 历史也不显示名字首字母。
...
- **边界计划**：Go 契约/迁移测试证明兼容；真实 Web 证明用户可见图标；macmini 重建证明旧数据保留。
...
5. **REPO_REQUIRED** — OpenAPI、Go/iOS/Web 测试、完整 build/race/vet、Docker build、许可证声明、对抗式评审、独立封闭合约评审和最终 macmini 旅程都通过。
...
6. **旅程新鲜度** — 那次旅程之后没有改动生产代码；只改了这份证据文档。最终的 macmini 重放仍是 S7 门禁。
...
- 验证者有意没有运行 Xcode 或部署；记录的 20 项 iOS 测试通过仍是 iOS 的证据，macmini 旅程仍是发布门禁。

## S7 — Mac mini 部署旅程

- 源码 commit `cf9f3af` 已推送到 `origin/main`，同步了评审过的运行时文件，并原地重建了既有的 Docker Compose 服务。
- 容器 `ferry` 在 `10.0.0.2:42817` 上运行，镜像 `sha256:c67deffa1aad731f5783ddab0962e918cd31857e335c5ab14f7deb454d23f06f`；启动完成，没有迁移错误。
...
- 证据截图：`/private/tmp/ferry-device-icons-macmini-cf9f3af-timeline.png`、`/private/tmp/ferry-device-icons-macmini-cf9f3af-devices.png`。
- 录制：`<browser-harness recordings>/ferry-device-icons-macmini-cf9f3af`。

...
**Decided（Max，2026-09-08）**：第 3 步可以自动完成。开启剪贴板同步后，别的设备发来的最新一条内容直接写入本机剪贴板；把本机剪贴板发出去仍然需要用户按一下。边界与依据见 `docs/clipboard-sync.md`。

发送方向始终由用户发起。三个平台都不允许后台读取系统剪贴板，因此不存在「自动监听」这一档，也不要尝试绕过。
...
- 第三方云托管账户体系。
- 自动读取并上传系统剪贴板。发送始终由用户发起：平台只允许前台读取剪贴板，且会就此提示用户，把它做成自动的等于骗人。
- 剪贴板历史。

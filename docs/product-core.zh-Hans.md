...
- **Decided（Max，2026-09-14）**：第一次访问 GitHub 仓库的用户，必须仅凭 README 就能部署 Ferry、用手机浏览器加入、复制一段文字并下载一个文件。默认安装方式是拉取已发布的镜像，配合一个小的部署包；不需要克隆仓库或安装 Go。
- **Decided（Max，2026-09-14）**：手机或电脑只需要浏览器。原生 App 是可选的，和 Web 旅程分开说明，不提供尚不存在的下载链接。
- **Decided（Max，2026-09-14）**：空时间线告诉用户可以发送什么，并提供 **Connect another device**。Devices 页面显示服务器地址、复制控件和在浏览器本地生成的二维码。二维码只包含浏览器当前的 origin，从不包含设备 token 或密码；仅本机可访问的地址不显示二维码。空状态由真实的消息状态决定，没有首次启动标记，也不虚构完成进度。
- **Decided（Max，2026-09-14）**：这条旅程保持既有边界：私有网络、主动发送、可选的共享密码，不新增 HTTP API 或数据库字段。

旅程的验收标准，记录在 `docs/release-process.zh-Hans.md`：

1. 从空目录按 README 操作，部署能启动，日志写出要打开的地址。
2. 真实手机浏览器扫码加入，覆盖无密码和有密码两种状态。
3. 一台设备发送的文字能在另一台设备上复制；图片能预览，文件能下载。
4. 重启后消息、文件、设备身份和密码都保留；现有源码部署切换到镜像后同样不丢失。

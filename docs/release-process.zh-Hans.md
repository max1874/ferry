...
| 7 | 隔离部署：按 README 从空目录安装，重启保留数据，现有源码部署切换到镜像，备份与恢复 | done（2026-09-14），有限定：先删除了 macmini 上原有的部署、数据卷和本地镜像，再在 macmini（arm64，已装好 OrbStack Docker 29.4.0 / Compose 5.1.2）的空目录里，用 `ferry-1.0.0-docker-compose.tar.gz` 按 README 快速开始操作。第 1–5 步约 13 秒，拉取了 `linux/arm64` 的 `sha256:3eba51a1…`，日志给出 `http://10.0.0.2:42817`。另一台 Mac 上的 Chrome 会话直接加入，Devices 页显示了二维码；同一台 Mac 上另一个隔离的 Chrome 会话也加入，两者之间互传了文字、一张 32 px 图片和一个文本文件，下载的文件逐字节一致。`docker compose restart` 之后，再执行 README 的备份和恢复命令，两个会话都保持加入，三条消息都在。设置了 `TZ=Asia/Shanghai` 的容器时间仍是 UTC。未覆盖：手机（见第 9 项）、复制控件、访问密码，以及源码部署的迁移（由 CI 覆盖） |

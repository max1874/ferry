...
| 7 | Isolated deployment from an empty directory following the README, restart keeping data, existing source deployment moved to the image, backup and restore | done (2026-09-14), with limits: after the earlier macmini deployment, volume and local image were removed, the README quick start ran on macmini (arm64, OrbStack Docker 29.4.0 / Compose 5.1.2, already installed) in an empty directory from `ferry-1.0.0-docker-compose.tar.gz`. Steps 1–5 took about 13 seconds, pulled `sha256:3eba51a1…` for `linux/arm64`, and the log named `http://10.0.0.2:42817`. A Chrome session on another Mac joined directly and showed the Devices QR code; a second, isolated Chrome session on that Mac joined, and text, a 32 px image and a text file crossed between the two, the downloaded file matching byte for byte. `docker compose restart`, then the README backup and restore commands, kept both sessions joined with all three messages. The container clock stayed UTC with `TZ=Asia/Shanghai` set. Not covered: a phone (see item 9), the Copy control, the password, and moving a source deployment (CI covers that move) |

...
- Ferry 是一个尚未定义产品范围、尚未创建 Xcode 工程的 iOS App。
- 在用户说明产品前，不要猜测功能、产品名含义、Bundle ID、开发者 Team、最低系统版本、商业模式、后端、图标体系、语言范围或发布方式。
- 可以调研、设计和搭建用户明确要求的基础设施；任何会提前锁定产品方向的选择都要等用户拍板。
...
- 默认在当前工作树工作。除非用户明确要求，不自行建分支、commit、push、开 PR 或发布。
- 复杂功能先冻结有限、可验证的 ship checklist；实现过程中发现新问题，先判断是否属于原目标，不能让调查替换目标。
- 修 bug 先找根因，再改机制；不要用重试、延时、吞错或 UI 遮挡来掩盖未知状态。

## iOS 工程原则

- 默认优先使用 Apple 原生框架与 SwiftUI；具体最低 iOS 版本和依赖策略在搭工程时确认并记录，当前不要预设。

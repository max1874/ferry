...
- 当前四个组件均已有 MVP。Android 已有真实设备安装/启动证据；跨 Web/iOS/Android 的最终家庭局域网旅程仍待用户回家后验收，不能用单测替代。
- 局域网 TLS 已由用户决定纳入范围，边界见 `docs/tls-lan.md`：`-tls` 用自签本地 CA 让浏览器把 Ferry 页面当作 secure context，这是剪贴板同步的前提。仍然不做公网部署与公共证书。
- 跨设备剪贴板同步已由用户决定纳入范围，边界见 `docs/product-core.md`：下行自动写入、上行一键发送、默认关闭。平台不允许后台读取剪贴板，不要尝试绕过。
- 当前不做公网部署、后台传输或应用商店发布；新增这些方向前必须由用户明确决定。

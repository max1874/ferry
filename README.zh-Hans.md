...
把下面这段话复制给能够在目标电脑上执行命令的 agent，就可以让它带你完成 Docker 部署。安装软件或获取系统权限时，可能需要你授权。

```text
请在这台电脑上用 Docker 部署 Ferry，让同一私有网络里的手机和其他电脑都能使用。请实际执行部署，不要只给操作说明。

先阅读 https://github.com/max1874/ferry/blob/main/README.zh-Hans.md 并查看 https://github.com/max1874/ferry/releases/latest，使用最新稳定版本的 Docker Compose 部署包及其对应的已发布镜像。

检查操作系统、CPU 架构、Docker Engine 和 Compose v2，以及是否已有 Ferry 部署。缺少依赖时，说明需要安装什么并征得我的同意。保留已有部署、配置和数据；升级前先备份。

新部署使用独立目录，保留部署包的目录结构，配置持久化存储和合适的私有局域网 IP，检查端口可用后拉取并启动 Ferry。如果有多个可能的网络，请问我设备使用哪一个。只在可信私有网络内提供服务，不要暴露到公网或关闭防火墙。

检查容器状态、日志和所选局域网地址的 HTTP 访问，解决部署错误。最后告诉我安装版本、部署目录、浏览器访问地址，以及停止、启动、升级和备份的命令。然后引导我进入 Devices → Connect another device，扫码接入并完成一次文字和文件传输。区分你已完成的验证和仍需我的手机或确认的步骤，不要把本机检查成功说成跨设备传输成功。
```

想自己执行命令？请看[快速开始](#快速开始)。
...
如果你的 `.env` 里 `COMPOSE_FILE` 包含 `compose.build.yaml`，请先从这一行里去掉它（保留 override 文件），否则 Compose 会继续构建源码，而不是使用镜像。消息、文件、设备身份和密码都会保留。如果想继续从源码构建，见[从源码构建镜像](#从源码构建镜像)。
...
在本仓库的克隆里，`compose.build.yaml` 会构建当前检出的源码，而不是拉取已发布的镜像。服务、数据卷和配置都保持不变。要让每条命令（包括备份脚本）都使用它，在 `.env` 里加上 `COMPOSE_FILE=compose.yaml:compose.build.yaml`（Windows 上用 `;` 分隔），然后运行 `docker compose up --build -d`。一次性的等价写法是：
